...
## 长任务执行规范（跨技能共用 · 低 token + 防重复扣费）

本节是「长任务怎么等、怎么不重复扣费」的**跨技能唯一定义**（2026-09-19 收口）。`dance`、`palace-girlfriend-trending`、`h3-drama-producer` 等技能一律引用本节，不各自重写。

### 1. 低 token 等待法（2026-09-19 用户指定）

适用：RunningHub 应用 / 工作流出片、舞蹈动作迁移等耗时数分钟到半小时以上的任务。

- **脚本后台运行**（非阻塞启动），全程**只在状态变化时输出一行**（`SUBMITTED` / `STATUS RUNNING` / `STATUS SUCCESS` / `OUTPUT_FILE:` / `COST:` / `RECORD:` 等，合计 4—6 行）。
- agent **每 3—5 分钟查一次**命令状态，且**回读时限制输出长度**（`output_character_count ≤ 500`）——一次 37 分钟任务轮询开销仅几百 token。
- **禁止高频轮询回读**（每几秒查一次、每次全量回读输出），这是 token 浪费的主要来源。
- 服务端轮询由脚本内部承担（脚本自身 15—20s 查一次），agent 只做低频状态确认，两者不冲突。

### 2. 防重复扣费（提交侧硬规矩）

**核心：超时不等于未送达。** POST 提交时如挂起或超时，请求体可能已完整到达服务端——任务已创建并开始生成，但客户端没收到含 `taskId` 的响应。此时重试会创建重复任务，**每次都真扣费**，且无「列出我的任务」API 可查孤立任务。

1. **提交一律用 `curl.exe --max-time`**，禁止用 Python `requests` / `urllib` 裸提交（两者都会无限挂起）。
2. **挂起 / 超时后绝不立即重试**——至少等 2—3 分钟再考虑；先按 §3 用 taskId 或任务记录对账。
3. **本技能已内置**：`scripts/runninghub_app.py` 的提交 / 查询 / 上传全部走 `curl` 封装（`--max-time` 60/30/120），无需手工加；自建脚本照此办理。
4. **零扣费失败的识别**：提交阶段报「文件不存在」类错误属零扣费，可隔几分钟直接重跑同一 payload；**已上报 taskId 的一律走续接，不重提**。

### 3. 中断 / 超时续接（不重提）

长任务脚本必须支持**续接模式**：只轮询 + 下载已提交任务，不重新提交。

- 超时退出时必须打印续接命令（含 `taskId`），例如 dance 的 `run_dance_transfer.py --task-id <taskId>`。
- 恢复后先查状态：已 `SUCCESS` 直接下载；`RUNNING`/`QUEUED` 继续等；只有**确认无 taskId** 才允许重新提交。

### 4. 上传与提交之间的同步间隔

参考图写入 RunningHub 工作区有**分钟级同步延迟**，上传后秒提交会在 LoadImage 节点报「No such file or directory」**零扣费失败**：

- 上传与提交之间**留 2—5 分钟**（`tools\run_t8balance.py` 默认 `--sync-wait 180`）。
- 失败后**隔几分钟重提同一 payload** 即可，不算 payload 错误，不必改图或改参数。
- 上传接口**内容寻址**：同一文件返回相同 hash 文件名，跨集复用无需重传；**上传链接仅一天有效**，隔天出片需重传（文件名不变，payload 无需改）。

### 5. 并发、实例与下载

- **账户同时最多 3 个任务**，超限返回 errorCode `421`，等 30s 重提同一 payload（零风险）。
- 实例档位按显存需求选；OOM 报 errorCode `805` 且**零扣费**，升档重跑即可（各技能 SKILL.md 有具体档位基准）。
- 结果 URL **仅 24 小时有效**，`SUCCESS` 后立即下载；URL 含空格时需 URL 编码（curl 会 rc=3）。
