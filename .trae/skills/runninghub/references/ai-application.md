...
脚本路径：`<本技能目录>\scripts\runninghub_app.py`（下文用 `<脚本>` 代替）
输出目录：当前项目的 `runninghub-output\` 子目录（下文用 `<输出>` 代替，指该目录的完整路径），文件名带时间戳。
...
Map user's modifications to `--node` and `--file` arguments (单行命令，语法跟随运行环境——Windows 用 PowerShell、Linux/macOS 用 Bash，不要用 `\` 换行):

```powershell
# 修改文本节点 + 上传图片
python <脚本> --run WEBAPP_ID --node "52:prompt=make her hair into a short bob cut" --file "39:image=<输出>photo.jpg" -o "<输出>app_result_20260829_153000.png"

# 只改文本
python <脚本> --run WEBAPP_ID --node "52:prompt=a boy with sunglasses" -o "<输出>app_result_20260829_153000.png"
```
...
# 只改文本运行
python <脚本> --run 1877265245566922800 --node "52:prompt=a boy with sunglasses" -o "<输出>app_20260829_153000.png"

# 上传文件 + 改文本运行
python <脚本> --run 1877265245566922800 --file "39:image=<输出>photo.jpg" --node "52:prompt=change hairstyle to short bob" -o "<输出>app_20260829_153000.png"

# 用更强的 GPU 实例运行
python <脚本> --run 1877265245566922800 --node "52:prompt=a girl dancing" --instance-type plus -o "<输出>app_20260829_153000.png"
```
