...
脚本路径：`d:\work\minimax_h3\.trae\skills\runninghub\scripts\runninghub_app.py`（下文用 `<脚本>` 代替）
输出目录：`d:\work\minimax_h3\runninghub-output\`（下文用 `<输出>` 代替），文件名带时间戳。
...
```powershell
# 推荐应用（默认）
python <脚本> --list --sort RECOMMEND --size 10

# 近 7 天最热
python <脚本> --list --sort HOTTEST --size 10 --days 7

# 最新应用
python <脚本> --list --sort NEWEST --size 10

# 第 2 页
python <脚本> --list --sort RECOMMEND --size 10 --page 2
```
...
**Present apps to the user with cover images**. For EACH app, deliver its cover image with a computer:// link, then describe it:

```
For each app in the list:
  1. 回复中写一条：1. **全能图片2.0** — 多功能图片生成 [封面](computer://<coverFile 本地路径>)
  2. Move to next app
After all apps, end with:
  "想试试哪个？告诉我编号就行！也可以说'下一页'看更多～"
```

Alternatively, if the list is long, you can deliver just the **first 3 covers** as links and list the rest as text.

Rules:
- ALWAYS deliver cover images via computer:// links — NEVER show cover URLs or bare file paths as text
...
**Before running the script**, ALWAYS tell the user in text first:
> "好的，开始运行 AI 应用啦！工作流生成通常需要几分钟，请稍等～ 🎬"

This is critical — AI app tasks are slow, and users need to know the task has started. Send the notification FIRST, then execute the script (建议后台运行).

Map user's modifications to `--node` and `--file` arguments (单行 PowerShell 命令，不要用 bash 的 `\` 换行):

```powershell
# 修改文本节点 + 上传图片
python <脚本> --run WEBAPP_ID --node "52:prompt=make her hair into a short bob cut" --file "39:image=d:\work\minimax_h3\runninghub-output\photo.jpg" -o "d:\work\minimax_h3\runninghub-output\app_result_20260829_153000.png"

# 只改文本
python <脚本> --run WEBAPP_ID --node "52:prompt=a boy with sunglasses" -o "d:\work\minimax_h3\runninghub-output\app_result_20260829_153000.png"
```
...
Same rules as standard API: deliver files via computer:// links, report cost, suggest next steps.
...
```powershell
# 查看应用的可修改节点
python <脚本> --info 1877265245566922800

# 只改文本运行
python <脚本> --run 1877265245566922800 --node "52:prompt=a boy with sunglasses" -o "d:\work\minimax_h3\runninghub-output\app_20260829_153000.png"

# 上传文件 + 改文本运行
python <脚本> --run 1877265245566922800 --file "39:image=d:\work\minimax_h3\runninghub-output\photo.jpg" --node "52:prompt=change hairstyle to short bob" -o "d:\work\minimax_h3\runninghub-output\app_20260829_153000.png"

# 用更强的 GPU 实例运行
python <脚本> --run 1877265245566922800 --node "52:prompt=a girl dancing" --instance-type plus -o "d:\work\minimax_h3\runninghub-output\app_20260829_153000.png"
```
