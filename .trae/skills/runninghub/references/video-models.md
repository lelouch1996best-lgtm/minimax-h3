...
**Before running the script**, ALWAYS notify the user in text first:
> "好嘞，开始用 XX模型 生成视频啦！一般需要几分钟，请稍等～ 🎬"

This is critical — video generation takes 1-5 minutes and users need to know the task has started. Send the notification FIRST, then execute the script (建议后台运行，脚本会自动轮询直到完成).
